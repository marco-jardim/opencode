...
export function delay(attempt: number, error?: SessionV1.APIError, random = Math.random()) {
  ...
  if (error) {
    ...
    if (headers) {
      ...
      return cap(exponential(attempt, random, initial, factor))
    }
  }

  return cap(Math.min(exponential(attempt, random, initial, factor), RETRY_MAX_DELAY_NO_HEADERS))
}

function exponential(attempt: number, random: number, initial = RETRY_INITIAL_DELAY, factor = RETRY_BACKOFF_FACTOR) {
  const base = initial * Math.pow(factor, attempt - 1)
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      ...
      // Unstable providers can remain retryable indefinitely, so cap retries to let the session return to idle.
