...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  // subagents don't need MCP tools — schemas are expensive (~50-100 tokens each × 20+ tools)
  const subagentSkipsMcp = input.agent.mode === "subagent" && !input.agent.options?.enableMcp
  const hasMcpResourceServer = Object.values(yield* mcp.clients()).some(
    (client) => !!client.getServerCapabilities()?.resources,
  )
  if (hasMcpResourceServer && !subagentSkipsMcp) {
  ...
  if (flags.experimentalCodeMode || subagentSkipsMcp) return tools
